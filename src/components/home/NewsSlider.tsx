...
export function NewsSlider({ className }: NewsSliderProps) {
    ...
    return (
        <div className={cn("overflow-hidden mb-6 rounded-3xl bg-muted/20 border border-white/5 shadow-xl", className)} ref={emblaRef}>
            <div className="flex">
                {noticias.map((noticia) => (
                    <div
                        key={noticia.id}
                        className="flex-[0_0_100%] min-w-0 relative h-44 cursor-pointer group"
                    ...
                    >
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-4 flex flex-col justify-end">
                            <div className="space-y-2">
                                ...
                                <h3 className="text-white font-extrabold text-lg leading-tight line-clamp-2 flex items-center gap-2 group-hover:text-primary/90 transition-colors">
                                    {noticia.titulo}
